...
def upload_compressed_file(compressed_file, dataset):
    _, file_extension = os.path.splitext(compressed_file.name)
    if file_extension == '.zip':
        with zipfile.ZipFile(compressed_file, 'r') as zip_ref:
            process_compressed_files(zip_ref, dataset)
    elif file_extension == '.tar' or file_extension == '.gz':
        with tarfile.open(fileobj=compressed_file, mode='r') as tar_ref:
            process_compressed_files(tar_ref, dataset)
    else:
        pass

def process_compressed_files(compressed_ref, dataset):
    parent_folder = Folder.objects.create(name=dataset.title, parent_folder=None, dataset=dataset)
    total_files = 0
    if isinstance(compressed_ref, zipfile.ZipFile):
        for file_name in compressed_ref.namelist():
            if not file_name.endswith('/'):
                total_files += 1
                create_file_from_compressed(compressed_ref, file_name, dataset, parent_folder)
    elif isinstance(compressed_ref, tarfile.TarFile):
        for file_info in compressed_ref.getmembers():
            if not file_info.isdir():
                total_files += 1
                create_file_from_compressed(compressed_ref, file_info, dataset, parent_folder)
    dataset.total_files = total_files
    dataset.save()



def create_file_from_compressed(compressed_ref, file_info, dataset, parent_folder):
    ...
    if isinstance(file_info, (zipfile.ZipInfo, tarfile.TarInfo)):
        ...
        for folder in folders[:-1]:
            parent_folder = Folder.objects.get_or_create(name=folder, parent_folder=parent_folder, dataset=dataset)[0]
    else:
    ...
    if parent_folder is None:
        parent_folder = Folder.objects.get_or_create(name=dataset.title, parent_folder=parent_folder, dataset=dataset)[0]
    ...
    upload_path = os.path.join(f'{dataset.id}', filename_without_extension)
